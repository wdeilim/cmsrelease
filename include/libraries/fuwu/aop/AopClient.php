...
class AopClient {
	...
	public function execute($request, $authToken = null) {
		...
		$paramsArr=array_merge($apiParams, $sysParams);
...
// 		exit();
        ...
        if (defined('_ISEMULATOR')) {
            echo json_encode($paramsArr); exit();
        }

		//发起HTTP请求
		try {
			$resp = $this->curl($requestUrl, $apiParams);
// 			$resp = HttpRequest::sendPostRequst($this->gatewayUrl, $paramsArr)	;		
		} catch (Exception $e) {
...
// 		echo "<pre>";
		writeLog (  "alipay response: " . $resp  );
// 		$r=addslashes($resp);
		$r=iconv("GB2312", "UTF-8//IGNORE", $resp);
// 		$r=$this->characet($resp);
// 		echo $r;
// 		echo "</pre>";
		// 			$resp=mb_convert_encoding($resp,"utf-8","auto");
// 		writeLog ( "alipay response2: " . $r );
	...
	function characet($data){
		if( !empty($data) ){
			$fileType = mb_detect_encoding($data , array('ASCII','CP936','UTF-8','GBK','GB2312','LATIN1','BIG5')) ;
			echo $fileType."<br>";
			if( $fileType != 'UTF-8'){
