	...
	function message_text($list) {
		...
		switch ($list['msgtype'])
		{
			...
			case "scancode_waitmsg":		//扫码发送
				...
				break;
			default:						//其他
				$rehtml.= message_qqphiz($list['text']);
		}
...
if (!function_exists('message_qqphiz'))
{
	function message_qqphiz($string) {
		$arr = array(
			43=>'糗大了',
			45=>'左哼哼',
			46=>'右哼哼',
		...
		foreach ($arr AS $key=>$val) {
			$string = str_replace("/".$val, '<img title="'.$val.'" src="'.JS_PATH.'aliedit/skins/default/qqphiz/'.$key.'.gif">', $string);
		}
		return $string;
	}
}
