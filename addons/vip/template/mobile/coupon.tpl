...
<div class="layout">
    <header>
        <img src="{#vippagebanner('coupon', $_A['vip_data']['banner'])#}" class="hd-banner">
    </header>
    <div class="wrap">
            ...
            <li class="item{#isto($list['view'],'0',' red')#}" data-id="{#$list['contentid']#}">
                ...
                <div class="bd">
                    ...
                    <div class="vip-action">
                        <div class="vip-miss">此优惠券已使用完毕！</div>
                    </div>
                    {#elseif ($list.startdate>$TIME || $list.enddate<$TIME)#}
                    <div class="vip-action">
                        <div class="vip-miss">此优惠券不在可使用期内！</div>
                    </div>
                    {#else#}
                    <div class="vip-action" id="vip-twocode">
                        ...
                        <div class="vip-qr"><img src="{#appurl('vip/qrcode/coupon')#}&sn={#$list['sn']#}" alt=""></div>
                    </div>
                    ...
                    <div class="vip-usenum">
                        {#if ($list['type_b']=="打折优惠券")#}
                        {#$list['type_b']#}【<span>{#$list['int_a']#}</span>折】<br>
                        {#else#}
                        {#$list['type_b']#}【抵<span>{#$list['int_b']#}</span>元】<br>
                        {#/if#}
                        此优惠券剩余【<span id="inum">{#$list['num']-$list['usenum']#}</span>张】<br>
                        此优惠券有效期【<span>{#date("Y-m-d",$list['startdate'])#}</span>至<span>{#date("Y-m-d",$list['enddate'])#}</span>】
                    </div>
    ...
    {#include file="./footer.tpl" _item="2"#}
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("button#butapply").click(function(){
            ...
            var s = "dosubmit=sub&money="+money+"&pass="+pass+"&sn="+sn;
            $.ajax({
                type: "POST",
                url: "{#appurl()#}",
                data: s,
