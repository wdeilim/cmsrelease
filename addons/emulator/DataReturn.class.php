...
class DataReturn{  // class start
    ...
    public function __construct($param=array()){
        $this->type = $this->exists($param,'type')? strtoupper($param['type']) : 'XML';   // 类型 JSON,XML,CALLBACK,ARRAY
        $this->xmlroot = $this->exists($param,'xmlroot')? $param['xmlroot'] : 'XML';   // xml root dom name
        ...
        $result = array();
        if (isset($param['format']) && $param['format'] == 'hide') {
            $result = $this->exists($param,'data')? $param['data'] : '';
        }else{
    ...
    private function json_return(){
        ...
        if(empty($this->callback)){
            ...
            return  $this->callback.'('.json_encode($this->returnData).');';
        }
    }
    ...
    private function xml_return(){
        ...
        return  $this->xml_encode($this->returnData,$this->xmlroot);
    }
    ...
    private function callback_return(){
        ...
        $d.= $this->callback."(".json_encode($this->returnData).");\r\n";
        $d.= "</script>";
        return $d;
    }
    ...
    private function array_return(){
        header('content-type:text/html;charset=utf-8');
        $d = '<pre>';
        $d.= print_r($this->returnData, true);
        $d.= '</pre>';
        return $d;
    }
