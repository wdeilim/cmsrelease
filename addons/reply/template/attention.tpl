...
<head lang="zh-CN">
    ...
    <title>关注时回复 - {#$func.f.title#} - {#$BASE_NAME#}</title>
...
<div class="wrapper">
    ...
    <div class="main cf custom-menu">
        <div class="mod">
            <div class="col-left">
                {#include file="left.tpl" _item=3 wx_level=$_A['al']['wx_level']#}
            </div>
            <div class="col-right">
                <div class="main-bd">
                    ...
                    <form action="{#$urlarr.now#}{#get_get()#}"  method="post" id="saveform">
                        <table class="table table-form">
                            <tbody>
                            <tr>
                                <td class="al-right">用户关注时回复</td>
                                <td class="form-reg">
                                    <select id="status" name="status">
                                        <option value="不启用">不启用</option>
                                        <option value="启用">启用</option>
                                    </select>
                                    {#if value($setting, 'attention|status')#}
                                    <script>$('#status').val('{#value($setting, 'attention|status')#}');</script>
                                    {#/if#}
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td style="line-height: 25px;" >
                                    {#tpl_form_aledit('content', value($setting,'attention|content'), 1)#}
                                </td>
                            </tr>
                            <tr>
                                <td></td>
